...
def run_planner(cfg: DictConfig):
    ...
    for idx in range(num_episodes):
        ...
        while step < max_steps and not done:
            ...
            # use detection model
            boxes, _ = object_detection(image_path=os.path.join(save_folder_obs, f"observed_{step}.png"), text_prompt=target_obj, annotation=True)
            if len(boxes)>0:
                h, w, _ = rgb[0].shape
            ...
            if obj_center is not None:
                ...
                optimal_object_center = None
                optimal_depth_val = None
                ...
                for gidx, goal_dict in enumerate(goals):
                    ...
                    for p, frame in enumerate(frames):
                        _, logits = object_detection(image_path=os.path.join(save_folder_imagine_step_goal, f"rendered_{p}.png"), text_prompt=target_obj, annotation=True)

                    max_idx = np.argmax(logits)
                    semantic_score = logits[max_idx]
                    ...
                    if semantic_score > best_semantic_score:
                        ...
                        optimal_object_center = object_center
                        optimal_depth_val = depth_val
                        # build the homogeneous pixel
            ...
            if is_direct:
                step_log["goal"] = {
                    ...
                    "goal": goal,
                    "path": path_habitat_exe,
                }
            else:
                step_log["imagined_goal"] = {
                    "object_center": optimal_object_center,
                    "depth_val": optimal_depth_val,
                    "semantic_score": best_semantic_score,
                    "goal": optimal_goal,
                    "path": path_habitat_exe,
                }

            step_log["trajectory"] = trajectory
    ...
    with initialize_config_dir(config_dir=cfg_path, version_base="1.2"):
        cfg = compose(
            ...
            overrides=[
                ...
                "model.encoder.use_camera_pose=False",
